...
contract Escrow is Ownable {
    ...
    function lzReceive(bytes calldata data)
        external
    {   
       require(msg.sender == lzAddress);
    ...
    function lzSend(EscrowStruct memory escrow) internal {
       bytes memory _payload = abi.encode(escrow);
         (uint128 gasLimit, uint128 gas) = lz.fees();
		 uint256 lzFee = lz.quote(dstEid, _payload, lz.createLzReceiveOption(gasLimit,gas),false);
		 require(msg.value>=lzFee,"lz: not enough gas");
		 lz.send{value: lzFee}(dstEid, _payload);
    }
	/**
