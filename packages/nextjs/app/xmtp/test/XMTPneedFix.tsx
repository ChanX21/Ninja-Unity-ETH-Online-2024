"use client";
// Duplicate message when sent and recieved.
...
const XMTPChatGPTBot = ({ onClose }: { onClose: () => void }) => {
    ...
    const [botXmtpClient, setBotXmtpClient] = useState<Client | null>(null);
    ...
    useEffect(() => {
        const initXMTP = async () => {
            if (signer) {
                try {
                    // Initialize user's XMTP client
                    const userClient = await Client.create(signer, { env: "production" });
                    setXmtpClient(userClient);

                    // Initialize bot's XMTP client
                    const botClient = await Client.create(botSigner, { env: "production" });
                    setBotXmtpClient(botClient);

                    // Create or load the conversation
                    const convo = await userClient.conversations.newConversation(CHATBOT_ADDRESS);
                    ...
                    await sendBotMessage(botClient, convo, initialGreeting);
    ...
    const sendBotMessage = async (botClient: Client, convo: any, message: string) => {
        const botConvo = await botClient.conversations.newConversation(address!);
        await botConvo.send(message);
    };

    const sendMessage = async () => {
        if (newMessage.trim() && conversation && botXmtpClient) {
            // Send user message
            ...
            await sendBotMessage(botXmtpClient, conversation, botResponse);
            setMessages(prevMessages => [
                ...prevMessages,
